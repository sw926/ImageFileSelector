...
internal object AppLogger {
    ...
    private const val TIMER_TAG = "TraceTime"
    ...
    fun v(msg: String, thr: Throwable?) {
        if (DEBUG) Log.v(TAG, buildMessage(msg), thr)
    }
    ...
    fun w(msg: String, thr: Throwable?) {
        if (DEBUG) Log.w(TAG, buildMessage(msg), thr)
    }
     ...
     */
    fun w(thr: Throwable?) {
        if (DEBUG) Log.w(TAG, buildMessage(""), thr)
    }
    ...
    fun e(msg: String, thr: Throwable?) {
        if (DEBUG) Log.e(TAG, buildMessage(msg), thr)
    }
